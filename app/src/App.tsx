...
export default function App() {
  ...
  // Initialize LIFF once; no-op when VITE_LIFF_ID is unset
  useEffect(() => { initLiff(); }, []);

  // Supabase Auth — check session on mount, listen for changes
  useEffect(() => {
  ...
  // Show login page if not authenticated via LIFF and no Supabase session
  if (!isLiffAuthed() && !session) return <LoginPage />;

  // Admin panel — full screen, replaces main app
  ...
  return (
    <div className="flex flex-col min-h-svh bg-[#FAFAF7] dark:bg-[#18181A]">
      ...
      <nav className="fixed bottom-0 inset-x-0 bg-white dark:bg-[#1A1A1C] border-t border-gray-100 dark:border-[#333336] flex justify-around items-stretch z-50 safe-area-pb">
        <NavItem
          active={tab === 'record'}
        ...
        />
        <NavItem
          active={tab === 'settings' || tab === 'pricing'}
